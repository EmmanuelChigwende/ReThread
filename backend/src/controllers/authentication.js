                ...
                if(email === userDetails.email){
                    if(!password){
                        ...
                        if(CheckHash(password,userDetails.password)){
                            ...
                            return res.status(200).json({message:`${userDetails.name} logged in successfully`,token:token})
                        }
                        else{
...
async function SignupUser(req,res) {
    try{
        ...
        if(!userEmail || !password || !name){
            return req.status(400).json({message:"please input email ,password and name"})
        }
        else{
            ...
            if(Exists){
                ...
                await UserModel.create({name:name,email:userEmail,password:hashedPassword})
